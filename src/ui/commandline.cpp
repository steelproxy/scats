...
std::string CommandLine::GetInput(bool lineEdit) // TODO: fix overflow
{
  ...
  while (charBuf != '\n') {
    ...
    if (checkPrintable(charBuf)) {
      ...
      // winsch(this->_wCommandLine, charBuf);       // insert character left of
      // current cursor pos wmove(this->_wCommandLine, _yPos, ++_xPos); // move
      // cursor forward one
      outPos++;
      wrefresh(this->_wCommandLine);
    } else {
      ...
      case KEY_UP: {
        ...
        {
          out =
              this->_commandHistory.at(this->_commandHistory.size() -
        ...
        } else if (commandHistoryIndex != 0) // if not at beginning of history
        {
          out = this->_commandHistory.at(
              --commandHistoryIndex); // decrement index and set std::string
                                      // buffer to previous command
        } else {
          out = this->_commandHistory.at(
              commandHistoryIndex); // set std::string buffer to current command
                                    // in this->_commandHistory
        }
...
void CommandLine::AddCommands() {
  ...
  _commands.emplace_back("save-settings", "Saves all settings.", SaveSettings);
